#!/bin/bash
#$ -cwd
#$ -l bluejay,mem_free=4G,h_vmem=10G,h_fsize=100G
#$ -N step2-trim-Bardy.singleCell
#$ -pe local 8
#$ -o ./logs/trim-Bardy.$TASK_ID.txt
#$ -e ./logs/trim-Bardy.$TASK_ID.txt
#$ -t 1-56
#$ -tc 5
#$ -hold_jid pipeline_setup,step1-fastqc-Bardy.singleCell
#$ -m a
echo "**** Job starts ****"
date
...
if [ TRUE == "TRUE" ] ; then 
	REPORT1=/dcl01/lieber/ajaffe/lab/libd_stem_timecourse/otherData/Bardy/FastQC/Untrimmed/${ID}/${FILEBASE1}_fastqc/summary.txt
	REPORT2=/dcl01/lieber/ajaffe/lab/libd_stem_timecourse/otherData/Bardy/FastQC/Untrimmed/${ID}/${FILEBASE2}_fastqc/summary.txt
	...
	if [[ $RESULT1 == "FAIL" || $RESULT2 == "FAIL" ]] ; then
		...
		mkdir -p /dcl01/lieber/ajaffe/lab/libd_stem_timecourse/otherData/Bardy/trimmed_fq
		...
		## rerun fastqc
		mkdir -p /dcl01/lieber/ajaffe/lab/libd_stem_timecourse/otherData/Bardy/FastQC/Trimmed/${ID}
		/dcl01/lieber/ajaffe/Emily/RNAseq-pipeline/Software/FastQC_v0.11.5/fastqc 		$FP $FU $RP $RU 		--outdir=/dcl01/lieber/ajaffe/lab/libd_stem_timecourse/otherData/Bardy/FastQC/Trimmed/${ID} --extract
	else
	...
	if [[ $RESULT1 == "FAIL" ]] ; then
		...
		mkdir -p /dcl01/lieber/ajaffe/lab/libd_stem_timecourse/otherData/Bardy/trimmed_fq
		OUT=/dcl01/lieber/ajaffe/lab/libd_stem_timecourse/otherData/Bardy/trimmed_fq/${ID}_trimmed.fastq
		...
		## rerun fastqc
		mkdir -p /dcl01/lieber/ajaffe/lab/libd_stem_timecourse/otherData/Bardy/FastQC/Trimmed/${ID}
		/dcl01/lieber/ajaffe/Emily/RNAseq-pipeline/Software/FastQC_v0.11.5/fastqc $OUT 		--outdir=/dcl01/lieber/ajaffe/lab/libd_stem_timecourse/otherData/Bardy/FastQC/Trimmed/${ID} --extract
	else
		echo "No trimming required!"
	fi
fi
