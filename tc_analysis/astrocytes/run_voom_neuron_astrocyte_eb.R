...
ffGene$FC = ifelse(ffGene$logFC<0, "DOWN", "UP")  ## down = on astro (h+r) is lower
...
pdf("pca_log2Rpkm_astrocytes.pdf", h=6,w=6)
par(mar=c(5,6,4,2),cex.axis=1.7,cex.lab=2,cex.main=2)
## PC 1 and 2
plot(pca1$x, bg=ifelse(pd$SPECIES=="HUMAN_RAT","#A6BDDB","#A31A28"), cex=2, main="Gene PCs",
	pch=21,
	xlab=paste0("PC1: ", pcaVars1[1], "% Var Expl"),
    ylab=paste0("PC2: ", pcaVars1[2], "% Var Expl"))
legend("topleft", paste0(levels(as.factor(pd$SPECIES))), col=c("#A31A28","#A6BDDB"), 
       pch = 15, cex=.9)
## PC 3 and 4
plot(pca1$x[,3],pca1$x[,4], bg=ifelse(pd$SPECIES=="HUMAN_RAT","#A6BDDB","#A31A28"), cex=2, main="Gene PCs",
	pch=21,
...
names(moduleGeneList) = c("Neuron+Astro > Neuron","Neuron+Astro < Neuron")
