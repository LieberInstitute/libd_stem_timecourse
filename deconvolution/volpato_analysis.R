...
## merge
pd = cbind(pd, stemPropEsts)
colData(rse_gene) = pd

## filter to neurons
rse_gene_neuron = rse_gene[,rse_gene$day == 80]

## model
mod = model.matrix(~cell_line + site, 
	data=colData(rse_gene_neuron))
modComp = model.matrix(~cell_line + site + 
	Neurons + NPC + Fetal_quiescent, data=colData(rse_gene_neuron))

## dge
dge = DGEList(counts = assays(rse_gene_neuron)$counts,
	genes = rowData(rse_gene_neuron)[,c("Symbol", "gene_type")])
dge = calcNormFactors(dge)

## modeling
vGene = voom(dge, mod,plot=FALSE)
vGeneComp = voom(dge, modComp, plot=FALSE)	

geneFit = lmFit(vGene, mod)
geneFit = eBayes(geneFit)
geneFitComp = lmFit(vGeneComp, modComp)
geneFitComp = eBayes(geneFitComp)

## extract stats 
siteStats = topTable(geneFit, coef = 3:6, sort="none", n= nrow(dge))
siteStatsComp = topTable(geneFitComp, coef = 3:6, sort="none", n= nrow(dge))

plot(siteStats$F, siteStatsComp$F)
sum(siteStats$P.Value < 1e-5) 
sum(siteStatsComp$P.Value < 1e-5)

siteStatsComp$Fdiff = siteStatsComp$F - siteStats$F
siteStatsComp$F_old = siteStats$F
siteStatsComp$pvalue_old = siteStats$P.Value

ind = order(siteStatsComp$Fdiff, decreasing=TRUE)[1:100]

y = vGene$E
sigStats = siteStatsComp[ind,]
boxplot(y[ind[1],] ~ rse_gene_neuron$site)
boxplot(y[ind[2],] ~ rse_gene_neuron$site)
plot(y[ind[1],] ~ rse_gene_neuron$Neurons)

ind2 = order(siteStatsComp$F_old, decreasing=TRUE)[1:100]
boxplot(y[ind2[1],] ~ rse_gene_neuron$site)
plot(y[ind2[1],] ~ rse_gene_neuron$Neurons)
...
rownames(celltype_vars) = rownames(r)
...
for (i in 94:103) {
	...
	celltype_vars[i-93,1] = anova(fit_site, fit_full)[2,6]
	celltype_vars[i-93,2] = anova(fit_day, fit_full)[2,6]
	celltype_vars[i-93,3] = anova(fit_line, fit_full)[2,6]
}

write.csv(signif(celltype_vars,3), file="volpato_nested_anova_cell_types.csv")
